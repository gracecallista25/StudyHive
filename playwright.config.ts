...
export default defineConfig({
  ...
  projects: [
    ...
    { name: 'preview-' + viewport.width + 'x' + viewport.height, testIgnore: ['**/registration-api.spec.ts', '**/profile-api.spec.ts'], use: { viewport, baseURL: 'http://127.0.0.1:5183' } },
    { name: 'api-' + viewport.width + 'x' + viewport.height, testMatch: ['**/registration-api.spec.ts', '**/profile-api.spec.ts'], use: { viewport, baseURL: 'http://127.0.0.1:5184' } },
  ]),
  webServer: [
