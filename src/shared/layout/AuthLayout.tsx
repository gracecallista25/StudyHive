...
import { ArrowUpRight, BookOpen, Sparkles, UsersRound } from 'lucide-react';
...
export function AuthLayout({ children, register = false }: AuthLayoutProps) {
  ...
  return (
    <div className={'auth-shell' + (register ? ' auth-register' : '')}>
      <aside className="auth-story" aria-label="Welcome to StudyHive">
        <a className="brand auth-brand" href="#login" aria-label="StudyHive login">
          <LeafMark />
          <div><span className="brand-name">StudyHive</span><p>HITSZ · Shenzhen</p></div>
        </a>
      ...
      <main className="auth-main">
        <div className="auth-topline"><span>{accountPrompt}</span><a href={accountHref}>{accountLink}<ArrowUpRight size={16} /></a></div>
        <div className="auth-content">{children}</div>
        <footer className="auth-footer">
          <div className="auth-avatar-stack"><StudentAvatar variant="lin" /><StudentAvatar variant="maya" /><StudentAvatar variant="yuna" /></div>
          <p>A place for curious minds.<br /><strong>Made for HITSZ students.</strong></p>
          <a href="#study-buddy">Explore demo<ArrowUpRight size={15} /></a>
        </footer>
      </main>
    </div>
  );
