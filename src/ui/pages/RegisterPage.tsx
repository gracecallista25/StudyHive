...
export function RegisterPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setFeedback('');
    setServerError('');
    if (Object.keys(next).length || !degree) return;
    ...
    const input: RegisterInput = {
      ...
      degree, major, grade: Number(year),
    };
  ...
  return <>
    <header className="auth-form-heading"><p className="auth-eyebrow">FIND YOUR PLACE IN THE HIVE</p><h2>Let’s start together.</h2><p>A few details to introduce yourself.</p></header>
    {!registrationEnabled && <div className="auth-preview-note"><Info size={17}/><p>Frontend preview. Accounts are not created or saved.<br/>Use fictional details to try the form.</p></div>}
    <form noValidate onSubmit={submit} aria-busy={pending} onChange={() => { setFeedback(''); setServerError(''); setErrors({}); }}>
      <fieldset className="auth-form register-form registration-fields" disabled={pending || complete}>
        ...
        <FormField label="Degree" name="degree" placeholder="Choose your degree" options={degreeOptions} selectValue={degree} onSelectChange={value => { setDegree(value as Degree); setMajor(''); setYear(''); }} required error={errors.degree}/>
