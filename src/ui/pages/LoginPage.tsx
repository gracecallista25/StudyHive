...
export function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setErrors(next); setFeedback(''); setFailure('');
    ...
    try {
      await loginAccount({ student_id: String(values.get('student_id')).trim(), password: String(values.get('password')) });
      form.reset();
  ...
  return <>
    <header className="auth-form-heading"><p className="auth-eyebrow">WELCOME BACK</p><h2>Good to see you.</h2><p>Your next study session starts here.</p></header>
    {!registrationEnabled && <div className="auth-preview-note"><Info size={17}/><p>Frontend preview. The backend is not connected.<br/>Use sample details to try the form.</p></div>}
    <form noValidate onSubmit={submit} aria-busy={pending} onChange={() => { setFeedback(''); setFailure(''); setErrors({}); }}>
